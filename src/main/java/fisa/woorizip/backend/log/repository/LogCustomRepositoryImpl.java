...
public class LogCustomRepositoryImpl implements LogCustomRepository {
    ...
    @Override
    public Page<Log> searchLogs(Long logId, String username, LocalDateTime startDate, LocalDateTime endDate, Pageable pageable) {
        ...
        Long count = searchLogsConditions(countQuery, logId, username, startDate, endDate).fetchOne();
        List<Log> content = searchLogsConditions(contentQuery, logId, username, startDate, endDate)
    ...
    private <T> JPAQuery<T> searchLogsConditions(JPAQuery<T> query, Long logId, String username, LocalDateTime startDate, LocalDateTime endDate) {
        return query.where(usernameContains(username), betweenDate(startDate, endDate), idEqual(logId));
    }
